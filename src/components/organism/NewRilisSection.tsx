...
const NewRilisSection = () => {
  ...
  const newFilmRelease = useFilmChillStore((state) => state.newFilmRelease);
    ...
    <section>
      <div className="px-8 py-8 sm:py-10 lg:py-16 sm:px-10 lg:px-16 overflow-x-hidden relative bg-background">
        ...
        >
          ...
          >
            {Array.isArray(newFilmRelease) && newFilmRelease.length > 0 ? (
              newFilmRelease.map((newReleaseItem) => (
                <SwiperSlide
                  key={newReleaseItem.id}
                  className="!overflow-visible relative z-0 hover:z-[999]"
                >
                  <CardFilmPotrait {...newReleaseItem} />
                </SwiperSlide>
              ))
            ) : (
